import { AboutComponent } from './about/about.component';
import { ArchiveComponent } from './archive/archive.component';
import { BasicPageComponent } from './basic-page/basic-page.component';
import { CareersListComponent } from './careers-list/careers-list.component';
import { CaseStudyComponent } from './case-study/case-study.component';
import { ContactComponent } from './contact/contact.component';
import { GridComponent } from './grid/grid.component';
import { HelpCenterComponent } from './help-center/help-center.component';
import { KnowledgebaseComponent } from './knowledgebase/knowledgebase.component';
import { LargeGridComponent } from './large-grid/large-grid.component';
import { MasonryComponent } from './masonry/masonry.component';
import { MessageCenterComponent } from './message-center/message-center.component';
import { OverviewComponent } from './overview/overview.component';
import { PagesComponent } from './pages/pages.component';
import { PositionDetailsComponent } from './position-details/position-details.component';
import { PostComponent } from './post/post.component';
import { PricingComponent } from './pricing/pricing.component';
import { ProjectComponent } from './project/project.component';
import { SupportTicketComponent } from './support-ticket/support-ticket.component';
import { TermsComponent } from './terms/terms.component';
import { TeamComponent } from './team/team.component';
import { Error400Component } from './error-400/error-400.component';
import { Error401Component } from './error-401/error-401.component';
import { Error403Component } from './error-403/error-403.component';
import { Error4041Component } from './error-404-1/error-404-1.component';
import { Error4042Component } from './error-404-2/error-404-2.component';
import { Error500Component } from './error-500/error-500.component';
import { Error503Component } from './error-503/error-503.component';
import { Error504Component } from './error-504/error-504.component';

export const containers = [PagesComponent, BasicPageComponent, AboutComponent, PricingComponent, ContactComponent, TermsComponent, HelpCenterComponent, KnowledgebaseComponent, MessageCenterComponent, SupportTicketComponent, CareersListComponent, PositionDetailsComponent, OverviewComponent, PostComponent, ArchiveComponent, GridComponent, LargeGridComponent, MasonryComponent, CaseStudyComponent, ProjectComponent, TeamComponent, Error400Component, Error401Component, Error403Component, Error4041Component, Error4042Component, Error500Component, Error503Component, Error504Component];

export * from './pages/pages.component';
export * from './basic-page/basic-page.component';
export * from './about/about.component';
export * from './pricing/pricing.component';
export * from './contact/contact.component';
export * from './terms/terms.component';
export * from './help-center/help-center.component';
export * from './knowledgebase/knowledgebase.component';
export * from './message-center/message-center.component';
export * from './support-ticket/support-ticket.component';
export * from './careers-list/careers-list.component';
export * from './position-details/position-details.component';
export * from './overview/overview.component';
export * from './post/post.component';
export * from './archive/archive.component';
export * from './grid/grid.component';
export * from './large-grid/large-grid.component';
export * from './masonry/masonry.component';
export * from './case-study/case-study.component';
export * from './project/project.component';
export * from './team/team.component';
export * from './error-400/error-400.component';
export * from './error-401/error-401.component';
export * from './error-403/error-403.component';
export * from './error-404-1/error-404-1.component';
export * from './error-404-2/error-404-2.component';
export * from './error-500/error-500.component';
export * from './error-503/error-503.component';
export * from './error-504/error-504.component';
